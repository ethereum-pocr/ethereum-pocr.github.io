...
window.addEventListener("load", ()=>{
  console.log("window loaded and doc ready state", document.readyState);
  _fullyLoaded=true
}, {once:true})
...
export function setupAuthNavigationGuard(router, store) {
  router.beforeEach(async (to, from, next) => {
    ...
    await fullLoaded();
    if (!fullLoaded) return next({name:"logs"})
    ...
    try {
      ...
      return next();
        ...
        console.error("Something happend when routing from "+from.name+" to "+to.name, error.message)
